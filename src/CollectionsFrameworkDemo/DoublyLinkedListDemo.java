...
class MyDoublyLinkedList {
    ...
    public void addFirst(Object input) {
        ...
        //새로운 노드의 다음 노드로 현재의 헤드를 지정합니다.
        newNode.next = head;

        //현재 헤드의 이전 노드로 새로운 노드 지정
        //head가 null일 경우 에러가 날 수 있기 때문에 if문을 걸어준다.
        if (head != null) {
            head.prev = newNode;
        }

        //새로운 헤드로 새로운 노드를 지정합니다.
        head = newNode;
        size++;
    ...
    public void add(int k, Object input) {
        ...
        if (k == 0) {
            ...
            //만약 꼬리를 추가할 경우 temp2가 null이라 에러가 날 수 있으므로 if문을 걸어줌
            if (temp2 != null) {
                temp2.prev = newNode;
            }
    ...
    public Object removeFirst() {
        ...
        //만약 노드가 헤드 하나밖에 없는 경우 헤드의 다음 노드는 없으므로 에러가 나기 때문에 if문을 걸어준다.
        if (head != null) {
            head.prev = null;
        }
        size--;
    ...
    public Object remove(int k) {
        if (k == 0) {
            ...
            //삭제 뒤 노드의 이전 노드를 삭제 앞 노드로 지정
            temp.next.prev = temp;

            //삭제된 데이터를 리턴 받기위해 저장
            Object returnData = todoDeleted.data;

            //삭제하려는 노드가 tail일 경우 tail 전 노드인 temp를 tail로 지정정
            if (todoDeleted == tail) {
                tail = temp;
            }
            todoDeleted = null;
    ...
    public class ListIterator {
        ...
        public Object previous() {
            if (next == null) {
                //lastReturned와 next를 tail로 지정하여 previous의 시작을 끊는다.
...
public class DoublyLinkedListDemo {
    public static void main(String[] args) {
        ...
        numbers.addLast(20);
        numbers.addLast(30);
        MyDoublyLinkedList.ListIterator i = numbers.listIterator();
